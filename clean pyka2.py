...
def log(username, bool_log=True, answer=None):
    global reply
    """
    ...
    while True:
        try:
            ...
            data = {'help_words': ['?', '??', '???', 'help', 'aide'],
                    ...
                    'yes_words': ['yep', 'yes', 'oui', 'yeah'],
                    'no_words': ['nop', 'no', 'non', 'nan'],
                    'politeness_word': ["s'il te plaît", "s'il te plait", 'stp', "s'il vous plaît", "s'il vous plait",
                                        'svp', 'please', 'pls'],
                    'politeness_end_word': ['merci', 'thank you', 'ty']}
...
def main():
    ...
    while True:
        if memory['lang'] == 'fr':
            reply = input("Que puis-je faire pour vous ?\t> ")
        elif memory['lang'] == 'en':
            reply = input("What can I do for you ?\t> ")
        clean_reply()
        if reply in words['bye_words']['good_night'] or words['bye_words']['bye']:
            if memory['lang'] == 'fr':
                if 6 <= int(time.strftime('%H')) <= 22:
...
memory = setup_memory()
words = setup_words()
true = words['yes_words']
false = words['no_words']
